...
// Project invitation — reuses the list invitation plumbing (token / accept
// flow) with project-flavored wording. A project invite grants access to every
// list in the project on acceptance (board #3).
...
export async function sendProjectInvitationEmail(data: ProjectInvitationData) {
  const subject = `${data.inviterName} invited you to the project "${data.projectName}"`
  const htmlBody = getProjectInvitationHtml(data)
  const textBody = getProjectInvitationText(data)
  const fromEmail = getFromEmail()
  ...
  if (process.env.NODE_ENV === "development" || !resend || !process.env.RESEND_API_KEY) {
    log.info({
      ...
      invitationUrl: data.invitationUrl,
      message: data.message || "No message",
    }, "📧 Project Invitation Email (Development Mode)")
    return
  }
  ...
  try {
    ...
    log.info({ id: emailData?.id, to: data.to }, '📧 Project invitation email sent successfully:')
    return emailData
  } catch (error) {
    log.error({ err: error }, 'Error sending project invitation email:')
    throw error
  }
}

function projectRoleAbility(role: string): string {
  return role === "manager"
    ? "manage the project, its lists, and its members"
    : "view and edit tasks across all of the project's lists"
}

function getProjectInvitationHtml(data: ProjectInvitationData): string {
  const roleDescription = projectRoleAbility(data.role)
  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <p><strong>${data.inviterName}</strong> has invited you to the project <strong>"${data.projectName}"</strong>.</p>
...
function getProjectInvitationText(data: ProjectInvitationData): string {
  const roleDescription = projectRoleAbility(data.role)

  return `
You've been invited to a project!

${data.inviterName} has invited you to the project "${data.projectName}".
