...
export default function FeatureRolloutsPage() {
  ...
  const load = useCallback(async () => {
    ...
    const google: FeatureFlag | null = data.flags.find((item: FeatureFlag) => item.key === 'google_tasks') ?? null
    setFlag(google)
    const included = google?.targets.filter(target => target.treatment === 'INCLUDE').map(target => target.user?.email).filter((email): email is string => Boolean(email)).join('\n') ?? ''
    const excluded = google?.targets.filter(target => target.treatment === 'EXCLUDE').map(target => target.user?.email).filter((email): email is string => Boolean(email)).join('\n') ?? ''
    setIncludedEmails(included)
    setExcludedEmails(excluded)
    if (google) setSavedDraft(buildFeatureRolloutDraft({ enabled: google.enabled, rolloutMode: google.rolloutMode, rolloutPercentage: google.rolloutPercentage, included, excluded }))
  }, [])
  ...
  const save = async () => {
    ...
    if (flag.enabled && flag.rolloutMode === 'ALL' && !confirm('Enable Google Tasks for all users except the exclusion list?')) return
    setSaving(true)
    try {
      const response = await fetch('/api/admin/features', {
        method: 'PUT',
