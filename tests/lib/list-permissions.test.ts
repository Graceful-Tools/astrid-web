...
describe('List Permissions - Permission Functions', () => {
  ...
  // Board sub-task #3: project membership grants access to the project's lists.
  describe('Project membership (sub-task #3)', () => {
    const baseList = (extra: object): TaskList => ({
      id: 'list-1',
      name: 'Project List',
      ownerId: 'other-owner',
      privacy: 'PRIVATE',
    ...
    it('grants member to a project member on the project list', () => {
      const list = baseList({ project: { ownerId: 'proj-owner', members: [{ userId: mockUser.id, role: 'member' }] } })
      expect(getUserRoleInList(mockUser, list)).toBe('member')
    })

    it('grants admin to a project admin', () => {
      const list = baseList({ project: { ownerId: 'proj-owner', members: [{ userId: mockUser.id, role: 'admin' }] } })
      expect(getUserRoleInList(mockUser, list)).toBe('admin')
    })

    it('grants admin to the project owner', () => {
      const list = baseList({ project: { ownerId: mockUser.id, members: [] } })
      expect(getUserRoleInList(mockUser, list)).toBe('admin')
    })

    it('returns null for a non-member on a private project list', () => {
      const list = baseList({ project: { ownerId: 'proj-owner', members: [{ userId: 'someone', role: 'member' }] } })
      expect(getUserRoleInList(mockUser, list)).toBeNull()
    })

    it('never LOWERS an existing list role: list admin + project member stays admin', () => {
      const list = baseList({
        listMembers: [{ id: 'lm', listId: 'list-1', userId: mockUser.id, role: 'admin', user: mockUser, createdAt: new Date(), updatedAt: new Date() }],
        project: { ownerId: 'proj-owner', members: [{ userId: mockUser.id, role: 'member' }] },
      })
      expect(getUserRoleInList(mockUser, list)).toBe('admin')
    })

    it('RAISES via project: list member + project admin becomes admin', () => {
      const list = baseList({
        listMembers: [{ id: 'lm', listId: 'list-1', userId: mockUser.id, role: 'member', user: mockUser, createdAt: new Date(), updatedAt: new Date() }],
        project: { ownerId: 'proj-owner', members: [{ userId: mockUser.id, role: 'admin' }] },
      })
      expect(getUserRoleInList(mockUser, list)).toBe('admin')
    })

    it('is a no-op when no project data is present (additive safety)', () => {
      const list = baseList({
        listMembers: [{ id: 'lm', listId: 'list-1', userId: mockUser.id, role: 'member', user: mockUser, createdAt: new Date(), updatedAt: new Date() }],
      })
      expect(getUserRoleInList(mockUser, list)).toBe('member')
