...
export const ChatMessageBubble = React.memo(function ChatMessageBubble({
  ...
  if (message.secureFiles?.length) {
    for (const file of message.secureFiles) {
      attachments.push({
        fileId: file.id,
  ...
  // Legacy direct URL attachments (skip if secureFiles already covers this message)
  if (message.attachmentUrl && !message.secureFiles?.length) {
    attachments.push({
