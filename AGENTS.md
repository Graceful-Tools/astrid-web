...
**Note:** This file is for **Codex CLI only** (local development). Cloud AI agents should read **ASTRID.md** for project context.

---

## Permissions Setup

**CRITICAL**: Codex reads permissions from `.Codex/settings.local.json`.

- Always check `.Codex/settings.local.json` for current permissions
- Template available at `.Codex/settings.json.example`
...
This will:
- Check if `.Codex/settings.local.json` exists
...
**Scenario 1: Local Codex session**
```
User: "ship it"
...
Codex detects comment and executes:
...
- When user comments "ship it" on the task, Codex merges and deploys
...
When user says "ship it" on an Astrid task (where an AI agent has been working), Codex executes the deployment workflow:

**Workflow:**
```
1. AI agent creates PR and posts link to task
2. User reviews PR preview and comments "ship it" on task
3. Codex merges PR to main
...
|-------|-------|----------|---------------|
| Codex | `Codex@astrid.cc` | Codex CLI | opus |
...
```bash
# Codex - Recommended (uses local CLI, no API key needed for terminal mode)
...
|------|---------|----------|
| **API** | `npx astrid-agent` | Cloud execution via Codex Agent SDK API |
| **Terminal** | `npx astrid-agent --terminal` | Local Codex CLI (remote control your local CLI) |
| **Webhook** | `npx astrid-agent serve` | Always-on servers (fly.io, VPS) |

### Terminal Mode (Multi-Provider Local Execution)

Processes tasks locally using Codex CLI, OpenAI API, or Gemini API with local tool execution. Routes to the correct provider based on task assignee.
...
# Codex uses local CLI, no API key needed for terminal mode
...
2. Routes based on assignee:
   - `Codex@astrid.cc` → Codex CLI (local)
...
5. Codex sessions stored in `~/.astrid-agent/terminal-sessions.json`

### API Mode (Default)

Uses Codex Agent SDK API for cloud execution. Best for CI/CD and servers without Codex CLI.
...
| Agent Email | Provider | Default Model |
|-------------|----------|---------------|
| `Codex@astrid.cc` | Codex | Codex CLI (opus) |
...
|---------|----------|---------------|
| Execution | Cloud (Codex Agent SDK) | Local (Codex CLI) |
| Requires | API key only | Codex CLI installed |
...
- **[.Codex/README.md](./.Codex/README.md)** - Permissions configuration guide

---

*This file is for Codex CLI only. For project context, see ASTRID.md.*
